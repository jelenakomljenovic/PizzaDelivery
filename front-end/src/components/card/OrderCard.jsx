...
class OrderCard extends React.Component {
  ...
  render() {
    return this.state.dict && this.state.restaurant && this.state.customer ? (
      <div>
        <Card>
          <Typography variant="body1" style={{position : "relative", backgroundColor: "#C10A28", color:"white"}} >
            ...
            {this.props.userType === "customer" && !this.props.order.delivery ? (
              <IconButton size="small" style={{position : "absolute", right : "0", color:"white"}} onClick={this.deleteOrder}>
                <ClearIcon />
              </IconButton>
            ) : null}
          </Typography>
          <Divider />
          <CardContent>
            {Object.keys(this.state.dict).map(key => <Typography variant="body1" color="#000000" component="p" key={key}><i>{key}</i> ... x {this.state.dict[key]}</Typography>)}
            <Divider />
            <br />
            <Grid container direction="column" alignItems="flex-end" spacing={1}>
              <Grid item xs={12}>
                <Typography style={{color:"#228B22"}} variant="body1"><i>Subtotal : $ {this.props.order.price}</i></Typography>
              </Grid>
          ...
          <Collapse in={this.state.expanded} timeout="auto" unmountOnExit>
            <CardContent>
              ...
              {this.props.order.startTime !== null && !this.props.order.delivery ? (
                <Typography paragraph><i>On waiting...</i></Typography>
              ) : null}
              ...
              {this.props.order.startTime !== null && this.props.order.delivery && this.state.wrongAddress ? <Typography style={{color:"#C10A28"}} variant="body1">The address can't be loaded, please contact the restaurant for detailed route information!</Typography> : null}
